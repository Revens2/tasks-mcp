...
Flux : l'iPhone (application Rappels) synchronise une liste CalDAV privée sur le VPS
`vps-etude` ; `tasks-mcp` expose ces tâches en MCP (Streamable HTTP) à ChatGPT / un
...
Depuis 2026-09 : quand tu dis à ChatGPT « je regarderai ça plus tard », la tâche
créée contient dans ses notes un lien cliquable vers la conversation exacte
...
| nginx vhost `caldav-tasks` | hôte | `10.200.114.203:5232` (TLS, CA privée) | NetBird |
...
| nginx vhost `tasks-mcp` | hôte | `127.0.0.1:8793` + `10.200.114.203:8793` | NetBird |
| Sauvegarde (`tasks-backup.timer`) | systemd | quotidienne 03:10 | — |

Aucune écoute publique (10.0.0.80) sur ces ports : l'exposition est impossible au
