...
#
# SAN couverts : noms DNS NetBird (vps-etude.netbird.selfhosted, caldav-vps.netbird.selfhosted)
# + adresse NetBird 10.200.114.203. Valdité feuille 800 jours (limite Apple 825 j).
#
...
ADRESSE_NETBIRD="10.200.114.203"
NOMS="DNS:vps-etude.netbird.selfhosted,DNS:caldav-vps.netbird.selfhosted,IP:$ADRESSE_NETBIRD"
FORCE_LEAF="${1:-}"
...
if [ ! -f "$FEUILLE_DIR/privkey.pem" ] || [ "$FORCE_LEAF" = "--force-leaf" ]; then
    ...
    openssl req -new -key "$FEUILLE_DIR/privkey.pem" \
        -subj "/CN=vps-etude.netbird.selfhosted" -out /tmp/tasks-caldav.csr
