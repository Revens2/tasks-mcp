...
from tasks_gateway.oauth import EtatOAuthCorrompu, MagasinOAuth
...
def test_deux_emissions_simultanees_ne_se_perdent_pas(tmp_path):
    ...
    def _emettre(jeton: str) -> None:
        ...
        magasin.poser_jetons(
            {"jeton": jeton, "client_id": "c", "scopes": ["tasks:lecture"], "expire_a": int(time.time()) + 3600},
            {"jeton": jeton + "-r", "client_id": "c", "scopes": ["tasks:lecture"], "expire_a": int(time.time()) + 3600},
        )
...
def test_code_autorisation_one_shot(tmp_path):
    ...
    magasin = _magasin(tmp_path)
    magasin.poser_code("code-1", {"client_id": "c", "scopes": ["tasks:lecture"], "expire_a": int(time.time()) + 90})
