//! tasks-gateway-rs — facade Rust devant l'upstream Python fige (`:8791`).
//!
//! Assemble le routeur `tasks_gateway_rs::build_router` (aucune auth upstream
//! requise, isolation systemd comme le Python ; `x-tasks-mcp-acteur` injecte
//! apres authentification).
//!
//! Environnement (prefixe `TASKS_MCP_RS_*`) :
//! * `TASKS_MCP_RS_ISSUER` (defaut issuer prod, HTTPS requis),
//! * `TASKS_MCP_RS_UPSTREAM` (defaut `http://127.0.0.1:8791`, loopback requis),
//! * `TASKS_MCP_RS_PORT` (defaut `18992` canary ; `8792` a la bascule),
//! * `TASKS_MCP_RS_TOKEN` (>= 32 car.) OU `TASKS_MCP_RS_TOKEN_FILE`
...
fn load_static_token() -> Result<String, String> {
    if let Ok(v) = std::env::var("TASKS_MCP_RS_TOKEN") {
        let v = v.trim().to_string();
        if !v.is_empty() {
            if v.len() < 32 {
                return Err("TASKS_MCP_RS_TOKEN trop court (<32 car.)".to_string());
            }
            return Ok(v);
        }
    }
    let path = std::env::var("TASKS_MCP_RS_TOKEN_FILE")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or_else(|| "/opt/tasks-gateway-rs/.mcp_token".to_string());
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    mcp_observe::init("tasks-gateway-rs");

    if std::env::var("TASKS_MCP_RS_ISSUER")
        .unwrap_or_default()
        .trim()
        .is_empty()
    {
        // `set_var` est `unsafe` sur cette toolchain : init mono-thread.
        unsafe { std::env::set_var("TASKS_MCP_RS_ISSUER", ISSUER_DEFAULT) };
    }
    if std::env::var("TASKS_MCP_RS_UPSTREAM")
        .unwrap_or_default()
        .trim()
        .is_empty()
    {
    ...
    unsafe { std::env::set_var("TASKS_MCP_RS_TOKEN", &token) };
    ...
    tracing::info!(
        ...
        "tasks-gateway-rs prete (boucle locale uniquement)"
    );
