"""Serveur d'autorisation OAuth 2.1 de la passerelle tasks, colocalise avec la ressource.
...
REPERTOIRE_DEFAUT = Path("/srv/tasks/data/oauth")
...
FICHIER_CLIENTS = Path("/srv/tasks/data/oauth/clients.json")

PORTEE = "tasks:lecture"
...
PORTEE_ECRITURE = "tasks:ecriture"
PORTEES = [PORTEE, PORTEE_ECRITURE]
# Client synthetique portant le jeton statique des clients CLI.
CLIENT_STATIQUE = "tasks-mcp-cli-statique"


def portees_du_jeton_statique() -> list[str]:
    """Portees accordees au Bearer statique des clients CLI.

    Pilotees par `TASKS_MCP_TOKEN_SCOPES` (portees separees par des espaces),
    defaut `tasks:lecture`. Une portee inconnue est ignoree plutot que propagee.
    """
    brut = os.environ.get("TASKS_MCP_TOKEN_SCOPES", PORTEE).split()
...
def repertoire_oauth() -> Path:
    return Path(os.environ.get("TASKS_MCP_OAUTH_DIR", str(REPERTOIRE_DEFAUT)))


def fichier_clients() -> Path:
    """Registre des clients, par defaut sous le repertoire oauth envoye.

    Un seul repertoire pilotable (`TASKS_MCP_OAUTH_DIR`) doit suffire en
    exploitation : `TASKS_MCP_OAUTH_CLIENTS` reste disponible pour deroger.
    """
    explicite = os.environ.get("TASKS_MCP_OAUTH_CLIENTS")
    if explicite:
