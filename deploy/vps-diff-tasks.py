"""Diff contrat zero live : prod Python :8792 vs canary Rust :18992.
...
read-only `tasks_today`, refus local outil inconnu (-32000 des deux cotes).
...
PROD_FILE = sys.argv[1] if len(sys.argv) > 1 else "/srv/tasks/secrets/tasks.env"
CANARY_FILE = sys.argv[2] if len(sys.argv) > 2 else "/opt/tasks-gateway-rs/.mcp_token"
PROD = "http://127.0.0.1:8792"
CANARY = "http://127.0.0.1:18992"
...
def lire_prod_env(path):
    try:
        ...
        sys.exit(2)
    m = re.search(
        r"(?m)^\s*(?:export\s+)?TASKS_MCP_TOKEN\s*=\s*['\"]?([^'\"\r\n]+)['\"]?\s*$",
        content,
    )
    if not m:
...
# 4. tools/call reel read-only tasks_today
_, pt, _ = post(PROD, TOKEN_PROD,
                {"jsonrpc": "2.0", "id": 4, "method": "tools/call",
                 "params": {"name": "tasks_today", "arguments": {}}}, sessions["prod"])
_, ct, _ = post(CANARY, TOKEN_CANARY,
                {"jsonrpc": "2.0", "id": 4, "method": "tools/call",
                 "params": {"name": "tasks_today", "arguments": {}}}, sessions["canary"])
pe, ce = "error" in pt, "error" in ct
print(f"tasks_today: prod_erreur={pe} canary_erreur={ce}")
if pe != ce:
    diffs.append("tasks_today: erreur d'un seul cote")
